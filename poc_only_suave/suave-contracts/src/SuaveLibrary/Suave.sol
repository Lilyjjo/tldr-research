...
library Suave {
    ...
    address public constant BUILD_ETH_BLOCK =
    ...
    address public constant ETHCALL =
    ...
    address public constant FILL_MEV_SHARE_BUNDLE =
        0x0000000000000000000000000000000043200001;

    address public constant NEW_BID =
        0x0000000000000000000000000000000042030000;

    address public constant SIGN_ETH_TRANSACTION =
        0x0000000000000000000000000000000040100001;

    address public constant SIMULATE_BUNDLE =
        0x0000000000000000000000000000000042100000;

    address public constant SUBMIT_BUNDLE_JSON_RPC =
        0x0000000000000000000000000000000043000001;
    ...
    function isConfidential() internal view returns (bool b) {
        (bool success, bytes memory isConfidentialBytes) = IS_CONFIDENTIAL_ADDR
            .staticcall("");
        if (!success) {
    ...
    function buildEthBlock(
        ...
        (bool success, bytes memory data) = BUILD_ETH_BLOCK.staticcall(
            abi.encode(blockArgs, bidId, namespace)
        );
        if (!success) {
    ...
    function confidentialInputs() internal view returns (bytes memory) {
        (bool success, bytes memory data) = CONFIDENTIAL_INPUTS.staticcall(
            abi.encode()
        );
        if (!success) {
    ...
    function confidentialRetrieve(
        BidId bidId,
        string memory key
    ) internal view returns (bytes memory) {
        (bool success, bytes memory data) = CONFIDENTIAL_RETRIEVE.staticcall(
            abi.encode(bidId, key)
        );
        if (!success) {
    ...
    function confidentialStore(
        BidId bidId,
        string memory key,
        bytes memory data1
    ) internal view {
        (bool success, bytes memory data) = CONFIDENTIAL_STORE.staticcall(
            abi.encode(bidId, key, data1)
        );
        if (!success) {
    ...
    function ethcall(
        ...
        (bool success, bytes memory data) = ETHCALL.staticcall(
            abi.encode(contractAddr, input1)
        );
        if (!success) {
    ...
    function extractHint(
        ...
        (bool success, bytes memory data) = EXTRACT_HINT.staticcall(
    ...
    function fetchBids(
        ...
        (bool success, bytes memory data) = FETCH_BIDS.staticcall(
    ...
    function fillMevShareBundle(
        BidId bidId
    ) internal view returns (bytes memory) {
        require(isConfidential());
        (bool success, bytes memory data) = FILL_MEV_SHARE_BUNDLE.staticcall(
            abi.encode(bidId)
        );
        if (!success) {
    ...
    function newBid(
        ...
        (bool success, bytes memory data) = NEW_BID.staticcall(
            abi.encode(
                decryptionCondition,
                allowedPeekers,
                allowedStores,
    ...
    function signEthTransaction(
        bytes memory txn,
        string memory chainId,
        string memory signingKey
    ) internal view returns (bytes memory) {
        (bool success, bytes memory data) = SIGN_ETH_TRANSACTION.staticcall(
            abi.encode(txn, chainId, signingKey)
        );
        if (!success) {
    ...
    function simulateBundle(
        ...
        (bool success, bytes memory data) = SIMULATE_BUNDLE.staticcall(
            abi.encode(bundleData)
        );
        if (!success) {
    ...
    function submitBundleJsonRPC(
        string memory url,
        string memory method,
        bytes memory params
    ) internal view returns (bytes memory) {
        require(isConfidential());
        (bool success, bytes memory data) = SUBMIT_BUNDLE_JSON_RPC.staticcall(
    ...
    function submitEthBlockBidToRelay(
        string memory relayUrl,
        bytes memory builderBid
    ) internal view returns (bytes memory) {
        require(isConfidential());
        (bool success, bytes memory data) = SUBMIT_ETH_BLOCK_BID_TO_RELAY
            .staticcall(abi.encode(relayUrl, builderBid));
        if (!success) {
