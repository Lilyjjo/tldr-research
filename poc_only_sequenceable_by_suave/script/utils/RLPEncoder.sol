...
library RLPEncoder {
    ...
    function _rlpEncodeList(
        ...
        if (combinedLength < 56) {
            ...
            // copy over actual data
            uint index = 1 + bytesForLength;
            for (uint i = 0; i < data.length; ++i) {
                for (uint j = 0; j < data[i].length; ++j) {
                    encoded[index++] = data[i][j];
                }
            }
        }
    }
    ...
    function _rlpEncodeUint(
        ...
        if (data == 0) {
            encoded = new bytes(1);
            encoded[0] = bytes1(0x80);
        } else if (data < 128) {
            // bytes1 cast will grab first byte, need to shift over target byte and add encoding value
            encoded = new bytes(1);
            //encoded[0] = bytes1(bytes32(data) << 248); // TODO: some RLP encoding add 0x80 to this value, need to just match what Suave is expecting
